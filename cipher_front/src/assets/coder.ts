export const coder = (phrase: string, key: string, encrypt: boolean) => {
  ...
  for (let i = 0; i < phrase.length; i++) {
    ...
    if (
      ...
      letters.push(
        letterShift(phrase[i], keyArray[c % keyArray.length], encrypt),
      );
      c++;
    } else {
...
// let key: string = "dude";
//
// let result: string = coder(phrase, key, false);
